...
write.table(file="chrX_allpop_Het_exp_bialsites.txt",t(c("CHR","POPNAME","H_E_VARSITES","H_E_ALLSITES","NOVAR_IN_POP","N_TOT_IN_POP")),col.names = FALSE,row.names = FALSE)
...
for (POPNAME in c("Baka","Nzime","BaKola","Ngumba","AkaMbati","BaKiga","BaTwa","Nsua","BaKonjo","Karretjiepeople","GuiandGana","Juhoansi","Nama","Xun","BantuHerero",
            ...
            # WRITE TO FILE
            write.table(file="chrX_allpop_Het_exp_bialsites.txt",t(c("X",POPNAME,H_exp,H_exp_allsites,n_novar,n_tot)),col.names = FALSE,row.names = FALSE, append=TRUE)
            }
